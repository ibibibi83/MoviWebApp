from flask import Flask, request, render_template, redirect, jsonify, url_for
from data_manager import DataManager
from models import db, User, Movie
import os
from omdb_manager import fetch_movie_data

app = Flask(__name__)

basedir = os.path.abspath(os.path.dirname(__file__))
app.config['SQLALCHEMY_DATABASE_URI'] = f"sqlite:///{os.path.join(basedir, 'data/movies.db')}"
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

db.init_app(app)
data_manager = DataManager()


@app.route('/')
def index():
    """
    Render the homepage with a list of all users.

    Returns:
        HTML page displaying all users stored in the database.
    """
    users = data_manager.get_users()
    return render_template('index.html', users=users)


@app.route('/users', methods=['POST'])
def post_user():
    """
    Create a new user.

    Retrieves the user's name from the submitted form,
    validates the input, creates a new User object, and
    stores it in the database.

    Returns:
        Redirect to homepage if successful.
        Error message with status code 400 if name is missing.
    """
    name = request.form.get('name')

    if not name:
        return "Name is missing", 400

    new_user = User(name=name)
    data_manager.create_user(new_user)
    return redirect('/')


@app.route('/users/<int:user_id>/movies', methods=['GET'])
def get_user_movies(user_id):
    """
    Display all movies belonging to a specific user.

    Args:
        user_id (int): ID of the user whose movies should be displayed.

    Returns:
        HTML page showing the user and their list of movies.
    """
    movies = data_manager.get_movies(user_id)
    user = data_manager.get_user(user_id)
    return render_template("movies.html", user=user, movies=movies)


@app.route('/users/<int:user_id>/movies', methods=['POST'])
def add_movie_to_user(user_id):
    """
    Add a new movie to a user's movie list.

    The movie title is taken from a form submission.
    Movie details are fetched from the OMDb API. If the movie
    exists, it is stored in the database.

    Args:
        user_id (int): ID of the user to whom the movie will be added.

    Returns:
        Redirect to the user's movie list if successful.
        JSON error if title is missing.
        HTML response indicating movie already exists or was not found.
    """
    title = request.form.get('title')
    movies = data_manager.get_movies(user_id)
    user = data_manager.get_user(user_id)

    if not title:
        return jsonify({"error": "title is missing"}), 400

    api_data = fetch_movie_data(title)

    if api_data:
        print(title.lower())
        print(api_data.get("title").lower())

        if title.lower() == api_data.get("title").lower():
            return render_template("movies.html", movie_exist=True, user=user, movies=movies)

        movie = Movie(
            name=api_data.get("title"),
            director=api_data.get("director"),
            year=api_data.get("year"),
            poster_url=api_data.get("poster"),
            user_id=user_id
        )

        data_manager.add_movie(movie, user_id)
        return redirect(f'/users/{user_id}/movies')

    return render_template("movies.html", movie_not_found=True, user=user, movies=movies)


@app.route('/movies/<int:movie_id>/update', methods=['POST'])
def update_user_movie(movie_id):
    """
    Update the name of an existing movie.

    The new movie name is received via JSON request body.

    Args:
        movie_id (int): ID of the movie to update.

    Returns:
        JSON representation of the updated movie.
        Error response if required data is missing.
    """
    data = request.get_json()

    if not data or "name" not in data:
        return jsonify({"error": "name is missing"}), 400

    movie = data_manager.update_movie(movie_id, data["name"])
    return jsonify(movie.to_dict()), 200


@app.route('/movies/<int:movie_id>/delete', methods=['POST'])
def delete_user_movie(movie_id):
    """
    Delete a movie from the database.

    Args:
        movie_id (int): ID of the movie to delete.

    Returns:
        Redirect to the previous page or homepage if successful.
        JSON error response if deletion fails.
    """
    print(f"trying to delete movie movie_id")

    if data_manager.delete_movie(movie_id):
        return redirect(request.referrer or url_for("index"))
    else:
        return jsonify({"error": "error deleted movie"}), 400


if __name__ == '__main__':
    """
    Entry point of the Flask application.

    Creates all database tables if they do not exist
    and starts the Flask development server.
    """
    with app.app_context():
        db.create_all()

    app.run()